...
Address.prototype.network = function() {
  ...
  var answer;
  if (version === livenet.addressVersion || version === livenet.P2SHVersion || version === livenet.P2SHVersion_OLD)
    answer = livenet;
  else if (version === testnet.addressVersion || version === testnet.P2SHVersion || version === testnet.P2SHVersion_OLD)
    answer = testnet;
...
Address.prototype.isScript = function() {
  return this.isValid() && (this.version() === this.network().P2SHVersion || this.version() === this.network().P2SHVersion_OLD);
};

// returns true is the address is a pay-to-script (P2SH) old address type.
Address.prototype.isOldScript = function() {
  return this.isValid() && this.version() === this.network().P2SHVersion_OLD;
};
...
Address.prototype.getScriptPubKey = function() {
  ...
  else if (version === livenet.P2SHVersion || version === testnet.P2SHVersion || version === livenet.P2SHVersion_OLD || version === testnet.P2SHVersion_OLD)
...
Address.convertToNewScriptFormat = function(s) {
  var tempAddress = new Address(s);
  if(tempAddress.isOldScript())
  {
      var scriptPubKey = tempAddress.getScriptPubKey();
      return Address.fromScriptPubKey(scriptPubKey);
  }
  return tempAddress;
};
