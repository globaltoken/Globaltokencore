...
Block.prototype.getEquihashHeader = function getEquihashHeader() {
  ...
  var solution_size_buffer = new Buffer(("0000000" + (eh_solution_size).toString(16)).substr(-8), "hex");
  var buf = new Buffer(140+eh_solution_size+solution_size_buffer.toString().length);
  var ofs = 0;
  buf.writeUInt32LE(this.version, ofs);
  ofs += 4;
  this.prev_hash.copy(buf, ofs);
  ...
  ofs += 32;
  solution_size_buffer.copy(buf, ofs);
  ofs += solution_size_buffer.solution_size_buffer.toString().length;
