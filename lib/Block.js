...
Block.prototype.getEquihashHeader = function getEquihashHeader() {
  var eh_solution_size = this.equihash_solution_size;
  var solution_size_buffer = Buffer.from(eh_solution_size.toString(16), "hex");
  var buf = new Buffer(140+eh_solution_size+solution_size_buffer.byteLength);
  var ofs = 0;
  buf.writeUInt32LE(this.version, ofs);
  ofs += 4;
  this.prev_hash.copy(buf, ofs);
  ...
  ofs += 32;
  this.solution_size_buffer.copy(buf, ofs);
  ofs += solution_size_buffer.byteLength;
  this.equihash_solution.copy(buf, ofs);
  ofs += eh_solution_size;
  return buf;
};
...
Block.prototype.parse = function parse(parser, headerOnly) {
  ...
  if(((this.version & 0x3E00) == (5 << 9)) || ((this.version & 0x3E00) == (23 << 9)))
  ...
  if(this.version & 256)
  {
     ...
     if(auxpow_version & 0x00004000)
     {
        ...
        zhashPersonalize = parser.buffer(length); // auxpow2.0: Zhash personalize String 
     }
     ...
     var merkle_index = parser.word32le(); // Auxpow 1.0: Merkle Index
     ...
     var auxpow_chain_index = parser.word32le(); // Auxpow 1.0: Chain Index
     ...
     if(auxpow_version & 0x00002000)
     {
       ...
       var buffer = [];
       buffer.push(Buffer.from(auxpow_header_version.toString(16), 'hex'));
       buffer.push(Buffer.from(auxpow_header_prevhash, 'hex'));
       buffer.push(Buffer.from(auxpow_header_merkle_root, 'hex'));
       buffer.push(Buffer.from(auxpow_header_hashreserved, 'hex'));
       buffer.push(Buffer.from(auxpow_header_timestamp.toString(16), 'hex'));
       buffer.push(Buffer.from(auxpow_header_bits.toString(16), 'hex'));
       buffer.push(Buffer.from(auxpow_header_nonce, 'hex'));
       buffer.push(Buffer.from(length.toString(16), 'hex'));
       buffer.push(Buffer.from(auxpow_header_solution, 'hex'));
       auxpow_parent_block = Buffer.concat(buffer);
     }
     else
     {
       var auxpow_header_version = parser.word32le();
       ...
       buffer.push(Buffer.from(auxpow_header_timestamp.toString(16), 'hex'));
       buffer.push(Buffer.from(auxpow_header_bits.toString(16), 'hex'));
       buffer.push(Buffer.from(auxpow_header_nonce.toString(16), 'hex'));
       auxpow_parent_block = Buffer.concat(buffer);       
     }
    
     // Create the Buffer
     auxpow_buffer.push(Buffer.from(auxpow_header_version.toString(16), "hex"));
     if(auxpow_version & 0x00004000)
        auxpow_buffer.push(Buffer.from(zhashPersonalize, 'utf8'));
     auxpow_buffer.push(merkletx_tx.serialize());
     auxpow_buffer.push(Buffer.from(auxpow_merkletx_hashblock, 'hex'));
     auxpow_buffer.push(Buffer.from(merklebranch_count.toString(16), 'hex'));
     for(var i = 0; i < merklebranch_count; i++)
     {
        auxpow_buffer.push(Buffer.from(merklebranch[i], 'hex'));
     }
     auxpow_buffer.push(Buffer.from(merkle_index.toString(16), 'hex'));
     auxpow_buffer.push(Buffer.from(chain_merklebranch_count.toString(16), 'hex'));
     for(var i = 0; i < chain_merklebranch_count; i++)
     {
        auxpow_buffer.push(Buffer.from(chain_merklebranch[i], 'hex'));
     }
     auxpow_buffer.push(Buffer.from(auxpow_chain_index.toString(16), 'hex'));
     auxpow_buffer.push(Buffer.concat(auxpow_parent_block));
  }
