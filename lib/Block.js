...
function Block(data) {
  ...
  this.equihash_nonce = data.bignonce || util.NULL_HASH;
  this.equihash_solution = data.ehsolution || null;
...
Block.prototype.getEquihashHeader = function getEquihashHeader(solution) {
  var buf = new Buffer(140);
  var solutionbuffer = new Buffer(solution, 'hex');
  var ofs = 0;
  buf.writeUInt32LE(this.version, ofs);
  ofs += 4;
  this.prev_hash.copy(buf, ofs);
  ...
  buf.writeUInt32LE(this.bits, ofs);
  ofs += 4;
  buf.writeUInt32LE(this.equihash_nonce, ofs);
  ofs += 32;
  return Buffer.concat([buf, solutionbuffer]);
};
...
Block.prototype.parse = function parse(parser, headerOnly) {
  ...
  if(this.version & 256)
  {
     ...
     if(auxpow_version & 0x00002000)
     {
       ...
       var buffer = [auxpow_header_version, auxpow_header_prevhash, auxpow_header_merkle_root, auxpow_header_hashreserved, auxpow_header_timestamp, auxpow_header_bits, auxpow_header_nonce, auxpow_header_solution];
     ...
     else
     {
       ...
       var buffer = [auxpow_header_version, auxpow_header_prevhash, auxpow_header_merkle_root, auxpow_header_timestamp, auxpow_header_bits, auxpow_header_nonce];
       auxpow_parent_block = Buffer.concat(buffer);       
     }
    
     // Finalize auxpow into the variable
     this.auxpow.push(auxpow_version);
     if(auxpow_version & 0x00004000)
       this.auxpow.push(zhashPersonalize);
     this.auxpow.push(merkletx_tx);
     this.auxpow.push(auxpow_merkletx_hashblock);
     this.auxpow.push(merklebranch);
     this.auxpow.push(merkle_index);
     this.auxpow.push(chain_merklebranch);
     this.auxpow.push(auxpow_chain_index);
     this.auxpow.push(auxpow_parent_block);
  }
...
Block.prototype.calcHash = function calcHash(isequihash, eh_solution) {
  var header;
  if(isequihash)
     header = this.getEquihashHeader(eh_solution);
  else
...
Block.prototype.getHash = function getHash(isequihash, eh_solution) {
  if (!this.hash || !this.hash.length) this.hash = this.calcHash(isequihash, eh_solution);
