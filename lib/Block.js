...
Block.prototype.getEquihashHeader = function getEquihashHeader() {
  ...
  var solution_size_buffer = new Buffer(eh_solution_size.toString(16), "hex");
...
Block.prototype.parse = function parse(parser, headerOnly) {
  ...
  if(this.version & 256)
  {
     ...
     if(auxpow_version & 0x00002000)
     {
       ...
       var buffer = [];
       buffer.push(new Buffer(auxpow_header_version.toString(16), 'hex'));
       ...
       buffer.push(new Buffer(auxpow_header_timestamp.toString(16), 'hex'));
       buffer.push(new Buffer(auxpow_header_bits.toString(16), 'hex'));
       buffer.push(new Buffer(auxpow_header_nonce, 'hex'));
       buffer.push(new Buffer(length.toString(16), 'hex'));
     ...
     else
     {
       ...
       buffer.push(new Buffer(auxpow_header_timestamp.toString(16), 'hex'));
       buffer.push(new Buffer(auxpow_header_bits.toString(16), 'hex'));
       buffer.push(new Buffer(auxpow_header_nonce.toString(16), 'hex'));
       auxpow_parent_block = Buffer.concat(buffer);       
     }
    
     // Create the Buffer
     auxpow_buffer.push(new Buffer(auxpow_header_version.toString(16), "hex"));
     if(auxpow_version & 0x00004000)
     ...
     auxpow_buffer.push(new Buffer(merklebranch_count.toString(16), 'hex'));
     for(var i = 0; i < merklebranch_count; i++)
     {
        auxpow_buffer.push(new Buffer(merklebranch[i], 'hex'));
     }
     auxpow_buffer.push(new Buffer(merkle_index.toString(16), 'hex'));
     auxpow_buffer.push(new Buffer(chain_merklebranch_count.toString(16), 'hex'));
     for(var i = 0; i < chain_merklebranch_count; i++)
     {
        auxpow_buffer.push(new Buffer(chain_merklebranch[i], 'hex'));
     }
     auxpow_buffer.push(new Buffer(auxpow_chain_index.toString(16), 'hex'));
