...
Block.prototype.parse = function parse(parser, headerOnly) {
  ...
  if(this.version & 256)
  {
     ...
     else
     {
       ...
       ofs += 4;
       auxpow_parent_block = Buffer.concat(buf);       
     }
    
     // Create the Buffer
     var versionbuffer = new Buffer(4);
     buf.writeUInt32LE(auxpow_header_version, 0);
     ...
     var merke_index_buffer = new Buffer(4);
     buf.writeInt32LE(merkle_index, 0);
     auxpow_buffer.push(merke_index_buffer);
     ...
     buf.writeInt32LE(auxpow_chain_index, 0);
