...
Block.prototype.getEquihashHeader = function getEquihashHeader() {
  ...
  ofs += 32;
  sizebuffer1.writeUInt8LE(this.equihash_solution_size, ofs);
  ofs += 1;
  switch (parseInt(sizebuffer1.toString('hex'), 16)) 
  {
    case 0xFD:
    {
         sizebuffer2.writeUInt16LE(this.equihash_solution_size, ofs);
         ofs += 2;
         endbuffer=Buffer.concat([sizebuffer1, sizebuffer2]);
    }

    case 0xFE:
    {
         sizebuffer2.writeUInt32LE(this.equihash_solution_size, ofs);
         ofs += 4;
         endbuffer=Buffer.concat([sizebuffer1, sizebuffer2]);
    }
    ...
    case 0xFF:
    {
         sizebuffer2.writeUInt64LE(this.equihash_solution_size, ofs);
         ofs += 8;
         endbuffer=Buffer.concat([sizebuffer1, sizebuffer2]);
    }
    ...
    default:
    {
         endbuffer=sizebuffer1;  
    }
  }
  this.equihash_solution.copy(solutionbuffer, ofs);
  ofs += this.equihash_solution_size;
  return Buffer.concat([buf, endbuffer, solutionbuffer]);
};
...
Block.prototype.parse = function parse(parser, headerOnly) {
  ...
  if(this.version & 256)
  {
     ...
     if(auxpow_version & 0x00002000)
     {
       ...
       var buffer = [];
       buffer.push(new Buffer(("0000000" + (auxpow_header_version).toString(16)).substr(-8), 'hex'));
       buffer.push(new Buffer(auxpow_header_prevhash, 'hex'));
       buffer.push(new Buffer(auxpow_header_merkle_root, 'hex'));
       buffer.push(new Buffer(auxpow_header_hashreserved, 'hex'));
       buffer.push(new Buffer(("0000000" + (auxpow_header_timestamp).toString(16)).substr(-8), 'hex'));
       buffer.push(new Buffer(("0000000" + (auxpow_header_bits).toString(16)).substr(-8), 'hex'));
       buffer.push(new Buffer(auxpow_header_nonce, 'hex'));
       buffer.push(new Buffer(("0000000" + (length).toString(16)).substr(-8), 'hex'));
       buffer.push(new Buffer(auxpow_header_solution, 'hex'));
     ...
     else
     {
       ...
       buffer.push(new Buffer(("0000000" + (auxpow_header_timestamp).toString(16)).substr(-8), 'hex'));
       buffer.push(new Buffer(("0000000" + (auxpow_header_bits).toString(16)).substr(-8), 'hex'));
       buffer.push(new Buffer(("0000000" + (auxpow_header_nonce).toString(16)).substr(-8), 'hex'));
       auxpow_parent_block = Buffer.concat(buffer);       
     }
    
     // Create the Buffer
     auxpow_buffer.push(new Buffer(("0000000" + (auxpow_header_version).toString(16)).substr(-8), 'hex'));
     if(auxpow_version & 0x00004000)
     {
        auxpow_buffer.push(new Buffer(("0000000" + (zhash_pers_length).toString(16)).substr(-8), 'hex'));
     ...
     auxpow_buffer.push(new Buffer(("0000000" + (merklebranch_count).toString(16)).substr(-8), 'hex'));
     for(var i = 0; i < merklebranch_count; i++)
     {
        auxpow_buffer.push(new Buffer(merklebranch[i], 'hex'));
     }
     auxpow_buffer.push(new Buffer(("0000000" + (merkle_index).toString(16)).substr(-8), 'hex'));
     auxpow_buffer.push(new Buffer(("0000000" + (chain_merklebranch_count).toString(16)).substr(-8), 'hex'));
     for(var i = 0; i < chain_merklebranch_count; i++)
     {
        auxpow_buffer.push(new Buffer(chain_merklebranch[i], 'hex'));
     }
     auxpow_buffer.push(new Buffer(("0000000" + (auxpow_chain_index).toString(16)).substr(-8), 'hex'));
     auxpow_buffer.push(auxpow_parent_block);
     this.auxpow.push(Buffer.concat(auxpow_buffer));
  }
